...
vec3 computeSpotLight(SpotLight light, vec3 surfel_to_camera)
{
	...
	float cosPhi = dot(surfel_to_light, -light.spotDirection);
	float intensity = clamp((cosPhi-cos(light.outerCutOff)) / (cos(light.outerCutOff) - cos(light.innerCutOff)) , 0, 1);
